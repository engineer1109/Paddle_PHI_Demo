...
PADDLE_API Tensor graph_send_uv(const Tensor& x, const Tensor& y, const Tensor& src_index, const Tensor& dst_index, const std::string& message_op = "ADD");
...
PADDLE_API Tensor amax(const Tensor& x, const std::vector<int64_t>& dims = {}, bool keep_dim = false);

PADDLE_API Tensor amin(const Tensor& x, const std::vector<int64_t>& dims = {}, bool keep_dim = false);
...
PADDLE_API Tensor conv2d(const Tensor& input, const Tensor& filter, const std::vector<int>& strides, const std::vector<int>& paddings, const std::string& padding_algorithm, int groups, const std::vector<int>& dilations, const std::string& data_format, bool use_addto, int workspace_size_MB, bool exhaustive_search);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> generate_proposals_v2(const Tensor& scores, const Tensor& bbox_deltas, const Tensor& im_shape, const Tensor& anchors, const Tensor& variances, int pre_nms_top_n, int post_nms_top_n, float nms_thresh, float min_size, float eta, bool pixel_offset = true);

PADDLE_API Tensor graph_send_recv(const Tensor& x, const Tensor& src_index, const Tensor& dst_index, const std::string& reduce_op = "SUM", const IntArray& out_size = {0});

PADDLE_API Tensor graph_send_ue_recv(const Tensor& x, const Tensor& y, const Tensor& src_index, const Tensor& dst_index, const std::string& message_op, const std::string& reduce_op, const IntArray& out_size);
...
PADDLE_API Tensor max(const Tensor& x, const IntArray& dims = {}, bool keep_dim = false);
...
PADDLE_API Tensor mean(const Tensor& x, const IntArray& dims = {}, bool keep_dim = false);
...
PADDLE_API Tensor min(const Tensor& x, const IntArray& dims = {}, bool keep_dim = false);
...
PADDLE_API Tensor& put_along_axis_(Tensor& x, const Tensor& index, const Tensor& value, int axis, const std::string& reduce);
...
PADDLE_API Tensor reduce_prod(const Tensor& x, const IntArray& dims, bool keep_dim, bool reduce_all);
...
PADDLE_API Tensor repeat_interleave_with_tensor_index(const Tensor& x, const Tensor& repeats, int dim);
...
PADDLE_API Tensor sum(const Tensor& x, const IntArray& dims = {}, DataType out_dtype = DataType::UNDEFINED, bool keep_dim = false);
...
PADDLE_API std::tuple<Tensor, Tensor&, Tensor&, Tensor, Tensor, Tensor> sync_batch_norm_(const Tensor& x, const Tensor& scale, const Tensor& bias, Tensor& mean, Tensor& variance, float momentum, float epsilon, const std::string& data_layout, bool is_test, bool use_global_stats, bool trainable_statistics, bool fuse_with_relu);
...
PADDLE_API Tensor uniform_random(const IntArray& shape, DataType dtype, const Scalar& min, const Scalar& max, int seed, const Place& place = {});
...
PADDLE_API Tensor uniform_random_inplace(const Tensor& x, float min, float max, int seed, int diag_num, int diag_step, float diag_val);

PADDLE_API Tensor& uniform_random_inplace_(Tensor& x, float min, float max, int seed, int diag_num, int diag_step, float diag_val);
